...
KOKKOS_FUNCTION inline bool near(Vec2 a, Vec2 b, f32 eps = 1e-5f)
{
...
int main(int argc, char **argv)
{
  Kokkos::initialize(argc, argv);
  {
    ...
    MeshHit hit = photon::pt::intersect_mesh_bvh(mesh, bvh, ray, 1e-3f, 1e30f);
    assert(hit.hit);
    ...
    Vec3 expected_sn = photon::pt::normalize(n_h(0) * 0.5f + n_h(1) * 0.25f + n_h(2) * 0.25f);
    ...
    MeshHit hit2 = photon::pt::intersect_mesh_bvh(mesh2, bvh2, ray, 1e-3f, 1e30f);
    assert(hit2.hit);

    assert(near(hit2.shading_normal, hit2.n));
    assert(near(hit2.uv, Vec2{0.f, 0.f}));
    assert(hit2.material_id == 0u);
  }
  Kokkos::finalize();
  return 0;
}
